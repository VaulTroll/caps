...
def text_to_line(date, time, title, content):
    #크롤링한 내용을 하나로 합쳐 탭으로 항목 구분
    news_data = '{0}\t{1}\t{2}\t{3}'.format(date,time,title,content)
    return news_data
...
def naver_craw_def(date):
    ...
    while True:
        ...
        for newslist in naver_newslists:
            ...
            for data in naver_data:
                ...
                for content in naver_contents:
                    ...
                    n_data = text_to_line(n_date, n_time, n_title, n_content)
        ...
        #기사를 파일에 저장
        f = open('/Users/vaultroll/Desktop/caps/data/naver_data.txt','a',encoding='utf-8')
